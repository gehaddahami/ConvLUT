...
from init import random_restrict_fanin   # type: ignore
...
class UnswNb15NeqModel(nn.Module):
    def __init__(self, model_config):
        ...
        for i in range(1, len(self.num_neurons)):
            ...
            if i == 1:
                input_quant = QuantBrevitasActivation(QuantReLU(bit_width=model_config["input_bitwidth"], max_val=1.0, min_val=-2.0, quant_type=QuantType.INT, scaling_impl_type=ScalingImplType.CONST))
                output_quant = QuantBrevitasActivation(QuantReLU(bit_width=model_config["hidden_bitwidth"], max_val=1.61, min_val=-1.61, quant_type=QuantType.INT, scaling_impl_type=ScalingImplType.PARAMETER), pre_transforms=[bn])
                ...
                output_quant = QuantBrevitasActivation(QuantHardTanh(bit_width=model_config["output_bitwidth"], max_val=1.33, min_val=-1.33, narrow_range=False, quant_type=QuantType.INT, scaling_impl_type=ScalingImplType.PARAMETER), pre_transforms=[bn], post_transforms=[output_bias_scale])
